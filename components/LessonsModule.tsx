        ...
        <div className="bg-[#0f172a] border-b border-[#64748b]/40 shrink-0">
          <div className="p-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
               ...
               <div>
                  <h2 className="text-[18px] font-normal text-white uppercase tracking-tight text-high-contrast">{currentLesson.name}</h2>
                  <span className="text-[10px] font-normal text-[#3b82f6] uppercase tracking-widest mt-1 block">ORTAK_DERS_MATRİSİ</span>
               </div>
            </div>
          </div>
          <div className="px-4 py-2 flex gap-1 bg-black/40 border-t border-[#64748b]/20">
            {['ŞUBE_MATRİSİ', 'KADRO', 'ANALİZ'].map(tab => (
              <button key={tab} onClick={() => setActiveDetailTab(tab as any)} className={`flex-1 h-11 text-[9px] font-normal tracking-[0.2em] transition-all flex items-center justify-center relative overflow-hidden ${activeDetailTab === tab ? 'bg-[#334155] text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-3 bg-grid-hatched">
           {activeDetailTab === 'ŞUBE_MATRİSİ' && (
             <div className="space-y-1.5 pb-24">
                   ...
                   return (
                     <div key={cls.id} className={`flex items-center bg-[#1e293b]/60 border p-2 transition-all relative overflow-hidden h-16 shadow-lg ${isAssigned ? 'border-[#3b82f6]/40' : 'border-white/5 opacity-50 grayscale'}`}>
                        ...
                        <div className="flex flex-col min-w-[60px] mr-3 justify-center items-center">
                           <span className="text-[16px] font-normal text-white uppercase tracking-tighter leading-none">{cls.name}</span>
                           <span className="text-[6px] font-normal text-slate-500 uppercase mt-1 tracking-widest">{cls.shift}</span>
                        </div>
                        <div className="flex-1 flex items-center gap-2 min-w-0 pr-2">
                           {isAssigned ? (
                             <button onClick={() => { setStaffSearchTerm(''); setStaffPicker({ lessonId: currentLesson.id, classId: cls.id, currentId: assignment.teacherId }); }} className="flex-1 h-10 bg-black/40 border border-white/5 px-3 flex items-center justify-between hover:border-[#3b82f6] transition-all group overflow-hidden">
                                <span className="text-[10px] font-normal text-white uppercase truncate pr-2 group-hover:text-[#3b82f6]">{teacherObj?.name || 'PERSONEL_ATA'}</span>
                                <i className="fa-solid fa-user-plus text-[10px] text-slate-700"></i>
                             </button>
                           ) : (
                             <div className="flex-1 h-10 border border-dashed border-white/5 flex items-center justify-center"><span className="text-[8px] font-normal text-slate-700 uppercase">ATAMA_YOK</span></div>
                           )}
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                           {isAssigned && (
                             <div className="flex items-center gap-1 bg-black/60 p-0.5 border border-white/5">
                                ...
                                <span className="w-6 text-center text-[14px] font-normal text-[#fbbf24]">{assignment.hours}</span>
                ...
                }) : (<div className="py-24 text-center opacity-20"><p className="text-[12px] font-normal uppercase tracking-[0.4em]">BU SEVİYEDE ŞUBE TANIMLI DEĞİL</p></div>)}
             </div>
           )}
                 ...
                 {allTeachers.filter(t => (t.branchShorts || [t.branchShort]).some(b => standardizeBranchCode(b) === standardizeBranchCode(currentLesson.branch))).map(t => {
                    ...
                    return (
                       <div key={t.id} onClick={() => onNavigateToTeacher?.(t.id, 'ŞUBE')} className="bg-[#1e293b]/80 border border-white/10 p-3 hover:border-[#3b82f6] transition-all cursor-pointer relative shadow-lg group">
                          <h4 className="text-[13px] font-normal text-white uppercase truncate pr-4">{t.name}</h4>
                          <div className="flex items-center gap-2 mt-1">
                             <span className="text-[6px] font-normal text-slate-500 uppercase tracking-widest">{t.preferredShift || 'SABAH'}</span>
                             <span className="text-[6px] font-normal text-[#fbbf24] uppercase">{t.lessonCount}s KOTA</span>
                          </div>
                          <div className="mt-2 flex flex-wrap gap-1">
                             {assignedClasses.map(c => (<span key={c.id} className="text-[7px] font-normal px-1.5 py-0.5 bg-[#3b82f6]/10 border border-[#3b82f6]/20 text-[#3b82f6] uppercase">{c.name}</span>))}
                          </div>
                       </div>
                    );
                 })}
                   ...
                   <>
                    <div className="grid grid-cols-3 gap-2">
                       <div className="bg-[#1e293b] border border-white/10 p-3 flex flex-col items-center justify-center shadow-lg rounded-sm h-24">
                          <span className="text-[28px] font-normal text-[#fbbf24] leading-none">{lessonAnalysis.overallAvg}</span>
                          <span className="text-[6px] font-normal text-slate-500 uppercase tracking-widest mt-2">GENEL_ORTALAMA</span>
                       </div>
                       <div className="bg-[#1e293b] border border-white/10 p-3 flex flex-col items-center justify-center shadow-lg rounded-sm h-24">
                          <span className="text-[28px] font-normal text-[#3b82f6] leading-none">{lessonAnalysis.totalGraded}</span>
                          <span className="text-[6px] font-normal text-slate-500 uppercase tracking-widest mt-2">NOTLU_ÖĞRENCİ</span>
                       </div>
                       <div className="bg-[#1e293b] border border-white/10 p-3 flex flex-col items-center justify-center shadow-lg rounded-sm h-24">
                          <span className="text-[28px] font-normal text-green-500 leading-none">%{Math.round(((lessonAnalysis.distribution.range50to85 + lessonAnalysis.distribution.rangeAbove85) / Math.max(1, lessonAnalysis.totalGraded)) * 100)}</span>
                          <span className="text-[6px] font-normal text-slate-500 uppercase tracking-widest mt-2">BAŞARI_ORANI</span>
                       </div>
                    </div>
                    <div className="bg-[#1e293b]/60 border border-white/10 p-4 shadow-xl">
                       <span className="text-[8px] font-normal text-white uppercase tracking-[0.4em] block mb-4">NOT_DAĞILIM_DNA_MATRİSİ</span>
                       <div className="grid grid-cols-3 gap-2">
                          <div className="bg-black/40 border border-white/5 p-2 flex flex-col items-center gap-1.5">
                             <span className="text-[14px] font-normal text-red-500">{lessonAnalysis.distribution.rangeBelow50}</span>
                             ...
                             <span className="text-[5px] font-normal text-slate-600 uppercase tracking-widest text-center">KRİTİK (0-50)</span>
                          </div>
                          <div className="bg-black/40 border border-white/5 p-2 flex flex-col items-center gap-1.5">
                             <span className="text-[14px] font-normal text-[#3b82f6]">{lessonAnalysis.distribution.range50to85}</span>
                             ...
                             <span className="text-[5px] font-normal text-slate-600 uppercase tracking-widest text-center">STANDART (50-85)</span>
                          </div>
                          <div className="bg-black/40 border border-white/5 p-2 flex flex-col items-center gap-1.5">
                             <span className="text-[14px] font-normal text-green-500">{lessonAnalysis.distribution.rangeAbove85}</span>
                             ...
                             <span className="text-[5px] font-normal text-slate-600 uppercase tracking-widest text-center">İLERİ (85-100)</span>
                          </div>
                       </div>
                    </div>
                   </>
                ) : (<div className="py-24 text-center border-2 border-dashed border-white/5 opacity-20"><p className="text-[10px] font-normal uppercase tracking-[0.4em]">ANALİZ İÇİN NOT VERİSİ GEREKLİ</p></div>)}
             </div>
           )}
        </div>
             ...
             <div className="bg-[#0d141b] border-2 border-[#3b82f6] w-full max-w-md shadow-[0_0_100px_rgba(0,0,0,0.8)] flex flex-col animate-in zoom-in-95 duration-200 max-h-[85vh] rounded-sm overflow-hidden bg-grid-hatched">
                <div className="p-5 border-b border-white/10 flex justify-between items-center bg-[#162431] shrink-0">
                   <div className="flex-1 pr-4">
                      <h3 className="text-[13px] font-normal text-white uppercase tracking-widest leading-none">PERSONEL_BAĞLANTISI</h3>
                      <span className="text-[8px] font-normal text-[#3b82f6] uppercase mt-2 block tracking-[0.2em]">HEDEF: {allClasses.find(c => c.id === staffPicker.classId)?.name} ŞUBESİ</span>
                   </div>
                   ...
                   <div className="flex gap-2">
                      <div className="flex-1 relative">
                         ...
                         <input 
                           ...
                           className="w-full bg-black border border-white/10 pl-9 pr-4 py-2.5 text-[11px] font-normal text-white uppercase outline-none focus:border-[#3b82f6]"
                   ...
                   {showQuickAddTeacherInPicker && (
                      <div className="flex gap-2 animate-in slide-in-from-top-2">
                         <input 
                           ...
                           className="flex-1 bg-[#1e2e3d] border border-[#3b82f6]/40 px-3 py-2 text-[11px] font-normal text-white uppercase outline-none focus:border-[#fbbf24]"
                         ...
                         />
                         <button onClick={handleQuickAddTeacherInPicker} className="bg-[#fbbf24] text-black px-4 font-normal text-[10px] uppercase shadow-lg">EKLE</button>
                      </div>
                   )}
                </div>
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar space-y-1 p-3">
                   {filteredPickerStaff.length > 0 ? filteredPickerStaff.map(t => {
                      ...
                      return (
                        <button key={t.id} onClick={() => updateMatrixTeacher(staffPicker.lessonId, staffPicker.classId, t.id)} className={`w-full p-4 border transition-all h-20 relative flex items-center justify-between shadow-lg group ${isSelected ? 'bg-[#3b82f6]/10 border-[#3b82f6]' : 'bg-[#162431] border-white/5 hover:border-white/20'}`}>
                           ...
                           <div className="text-left flex flex-col justify-center min-w-0 mr-4">
                              <span className={`text-[14px] font-normal uppercase truncate whitespace-nowrap ${isSelected ? 'text-[#3b82f6]' : 'text-slate-300 group-hover:text-white'}`}>{t.name}</span>
                              <div className="flex items-center gap-2 mt-1.5">
                                 <span className="text-[7px] font-normal text-slate-500 uppercase tracking-widest">{t.branchShort}</span>
                                 <div className="w-1 h-1 rounded-full bg-slate-800"></div>
                                 <span className="text-[7px] font-normal text-[#fbbf24] uppercase">{t.preferredShift || 'SABAH'}</span>
                              </div>
                           </div>
                      ...
                      <div className="py-12 text-center opacity-30 border border-dashed border-white/10 flex flex-col items-center">
                         ...
                         <p className="text-[10px] font-normal uppercase tracking-[0.3em]">HOCA BULUNAMADI</p>
                      </div>
                   )}
                </div>
                
                <div className="p-4 bg-black/60 border-t border-white/5 shrink-0">
                   <p className="text-[7px] font-normal text-slate-600 uppercase tracking-widest leading-relaxed italic text-center">
  ...
  return (
    <div className="space-y-4 h-full flex flex-col overflow-hidden animate-slide-up relative px-1">
      <div className="flex flex-col gap-3 shrink-0 px-1">
         <div className="flex gap-2">
            <div className="flex-1 relative h-11"><i className="fa-solid fa-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-[#3b82f6] text-[10px]"></i><input type="text" placeholder="ENVANTER ARA..." className="w-full h-full bg-black border border-[#64748b]/40 pl-11 pr-4 text-[11px] font-normal uppercase text-white outline-none focus:border-[#3b82f6]" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
            <button onClick={() => { if(!editMode) onWatchModeAttempt(); else { setNewLessonData({ name: '', branch: '', hours: 2, selectedGrades: [], id: null }); setIsDrawerOpen(true); } }} className="px-6 h-11 bg-[#3b82f6] text-white font-normal text-[10px] uppercase tracking-widest shadow-lg active:scale-95 flex items-center gap-2 border border-white/10"><i className="fa-solid fa-plus text-[12px]"></i> EKLE</button>
         </div>
         <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1">
            {['TÜMÜ', ...availableGrades.map(String)].map(f => (
               <button key={f} onClick={() => setCategoryFilter(f)} className={`flex-1 min-w-[44px] h-9 text-[9px] font-normal uppercase tracking-widest border transition-all ${categoryFilter === f ? 'bg-[#3b82f6] border-[#3b82f6] text-white shadow-lg' : 'bg-black/40 border-[#354a5f] text-slate-500 hover:bg-[#1e2e3d]'}`}>{f === 'TÜMÜ' ? 'TÜMÜ' : `${f}.S`}</button>
            ))}
         </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar space-y-1.5 pb-24 px-1">
                 ...
                 <div onClick={() => setSelectedLessonId(l.id)} className={`bg-[#1e293b]/60 border border-white/5 h-full flex items-center justify-between px-4 transition-all cursor-pointer shadow-md active:scale-[0.98] relative overflow-hidden ${isMenuOpen ? '-translate-x-32' : 'hover:bg-slate-800'}`}>
                    ...
                    <div className="flex-1 flex items-center gap-4 min-w-0">
                       <div className="flex flex-col min-w-0">
                          <span className="text-[10px] font-normal text-slate-400 uppercase truncate transition-colors">{l.name}</span>
                          <div className="flex items-center gap-3 mt-0.5">
                             <span className="text-[5px] font-normal text-[#fbbf24] uppercase tracking-widest">{standardizeBranchCode(l.branch)}</span>
                             <div className="w-1 h-1 rounded-full bg-slate-700"></div>
                             <span className="text-[5px] font-normal text-slate-500 uppercase tracking-widest">{allClasses.filter(c => (c.assignments || []).some(a => a.lessonId === l.id)).length} ŞUBEDE</span>
                          </div>
                       </div>
                    </div>
                    <div className="flex items-center gap-4 shrink-0">
                       <div className="flex flex-col items-end mr-2">
                          <span className="text-[12px] font-normal text-slate-400 leading-none">{l.hours} s</span>
                          <span className="text-[5px] font-normal text-slate-500 uppercase mt-0.5 opacity-60">PLAN_SAATİ</span>
                       </div>
                       {editMode && (
         ...
         }) : (<div className="py-32 flex flex-col items-center justify-center opacity-20"><p className="text-[12px] font-normal uppercase tracking-[0.4em]">ENVANTER_BOŞ</p></div>)}
      </div>
      ...
      {lessonToDelete && (
        <div className="fixed inset-0 z-[9000] flex items-center justify-center bg-black/95 backdrop-blur-md px-4">
           <div className="bg-[#0d141b] border-2 border-red-600 p-8 max-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
              <h3 className="text-[14px] font-normal text-white uppercase tracking-widest mb-4">ENVANTER_SİLME_ONAYI</h3>
              <p className="text-[11px] font-normal text-slate-400 uppercase leading-relaxed mb-8">
                 ...
                 <span className="text-red-500 text-lg block mt-2 font-normal">{lessonToDelete.name}</span>
              </p>
              <div className="flex gap-4">
                 <button onClick={() => setLessonToDelete(null)} className="flex-1 h-12 border border-[#64748b] text-[#f1f5f9] font-normal text-[10px] uppercase hover:bg-white/5 transition-all">İPTAL</button>
                 <button onClick={executeDeleteLesson} className="flex-1 h-12 bg-red-600 text-white font-normal text-[10px] uppercase shadow-xl">EVET_SİL</button>
              </div>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[8000] flex items-center justify-center bg-black/95 backdrop-blur-md px-4">
           <div className="bg-[#0d141b] border-t-4 border-[#3b82f6] p-6 max-md w-full shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col bg-grid-hatched">
              <div className="flex justify-between items-center mb-8 shrink-0">
                 <div>
                    <h3 className="text-[14px] font-normal text-white uppercase tracking-widest leading-none">{newLessonData.id ? 'ENVANTER_GÜNCELLE' : 'YENİ_ENVANTER_KAYDI'}</h3>
                    <span className="text-[8px] font-normal text-[#3b82f6] uppercase tracking-[0.4em] mt-2 block">MÜFREDAT_DNA_GİRİŞİ</span>
                 </div>
              ...
              <div className="space-y-6">
                 <div className="flex gap-4">
                    <div className="flex-[3] space-y-1.5">
                       <label className="text-[7px] font-normal text-slate-500 uppercase tracking-widest ml-1">DERS ADI</label>
                       <input autoFocus className="w-full bg-black border border-white/10 p-3 text-[13px] font-normal text-white uppercase outline-none focus:border-[#3b82f6]" value={newLessonData.name} onChange={e => { const upper = e.target.value.toUpperCase(); setNewLessonData({ ...newLessonData, name: upper, branch: isBranchManual ? newLessonData.branch : standardizeBranchCode(upper.split(/\s+/)[0].substring(0,4)) }); }} />
                    </div>
                    <div className="flex-[1] space-y-1.5">
                       <label className="text-[7px] font-normal text-slate-500 uppercase tracking-widest ml-1">BRANŞ KODU</label>
                       <input className="w-full bg-black border border-white/10 p-3 text-[13px] font-normal text-[#fbbf24] text-center uppercase outline-none focus:border-[#fbbf24]" value={newLessonData.branch} onChange={e => { setNewLessonData({...newLessonData, branch: e.target.value.toUpperCase()}); setIsBranchManual(true); }} />
                    </div>
                 </div>
                 {!newLessonData.id && (
                   <div className="space-y-2.5">
                      <label className="text-[7px] font-normal text-slate-500 uppercase tracking-widest ml-1 block">SINIF_SEVİYESİ_DNA (ÇOKLU SEÇİM)</label>
                      <div className="grid grid-cols-4 sm:grid-cols-8 gap-1.5">
                         {[5, 6, 7, 8, 9, 10, 11, 12].map(g => (<button key={g} onClick={() => setNewLessonData(p => ({...p, selectedGrades: p.selectedGrades.includes(g) ? p.selectedGrades.filter(x => x !== g) : [...p.selectedGrades, g]}))} className={`h-11 border transition-all text-[11px] font-normal ${newLessonData.selectedGrades.includes(g) ? 'bg-[#3b82f6] border-[#3b82f6] text-white' : 'bg-black border-white/5 text-slate-600'}`}>{g}</button>))}
                      </div>
                   </div>
                 )}
                 <div className="flex gap-4 items-end">
                    <div className="flex-1 space-y-1.5">
                       <label className="text-[7px] font-normal text-slate-500 uppercase tracking-widest ml-1">PLAN_SAATİ</label>
                       <div className="flex items-center gap-3 bg-black border border-white/5 p-1">
                          ...
                          <span className="flex-1 text-center text-xl font-normal text-white">{newLessonData.hours}</span>
                    ...
                    }} className="flex-1 h-12 bg-[#3b82f6] text-white font-normal text-[12px] uppercase tracking-[0.3em] shadow-xl border border-white/10">{newLessonData.id ? 'GÜNCELLE' : 'MÜHÜRLLE_DNA'}</button>
                 </div>
              </div>
           </div>
        </div>
