        ...
        <div className="min-h-screen bg-[#0f172a] text-white p-4 md:p-6 animate-in fade-in duration-300 pb-20">
            ...
            <div className="flex justify-between items-center border-b border-white/10 pb-4 mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-[14px] font-normal uppercase tracking-[0.2em] leading-none text-slate-200">YOKLAMA RAPORU</h1>
                        <p className="text-[9px] text-slate-500 font-normal uppercase tracking-wider">GÜNLÜK ANALİZ</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <div className="relative group">
                        <input
                            ...
                            className="bg-[#1e293b] border border-white/10 text-white text-[10px] font-normal uppercase tracking-widest px-2 py-1.5 rounded-sm focus:outline-none focus:border-[#3b82f6] transition-all cursor-pointer w-32"
                        />
                    ...
                    <button onClick={() => window.print()} className="px-3 py-1.5 bg-[#3b82f6] text-white font-normal text-[10px] uppercase tracking-widest shadow-lg hover:brightness-110 active:scale-95 transition-all flex items-center gap-2 rounded-sm border border-white/10">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="bg-[#1e293b] border border-white/5 p-2 flex flex-col items-center justify-center rounded-sm relative overflow-hidden group h-20">
                    ...
                    <span className="text-[10px] font-normal text-slate-500 uppercase tracking-widest mb-1">TOPLAM ÖĞRENCİ</span>
                    <div className="text-xl font-normal text-white leading-none">{absenceData.totalAbsentStudents}</div>
                </div>

                <div className="bg-[#1e293b] border border-white/5 p-2 flex flex-col items-center justify-center rounded-sm relative overflow-hidden group h-20">
                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-amber-500/50"></div>
                    <span className="text-[10px] font-normal text-slate-500 uppercase tracking-widest mb-1">ETKİLENEN ŞUBE</span>
                    <div className="text-xl font-normal text-white leading-none">{absenceData.totalClassesWithAbsence}</div>
                </div>
                ...
                <div className="bg-[#1e293b] border border-white/5 p-2 flex flex-col items-center justify-center rounded-sm relative overflow-hidden h-20">
                    ...
                    <span className="text-[10px] font-normal text-slate-500 uppercase tracking-widest mb-1">RAPOR TARİHİ</span>
                    <div className="text-[11px] font-normal text-white uppercase">{selectedDate}</div>
                </div>
            </div>
            ...
            {sortedClassIds.length > 0 ? (
                        ...
                        return (
                            <div key={classId} className="bg-[#1e293b] border border-white/5 rounded-sm overflow-hidden">
                                ...
                                <div className="bg-[#0f172a] px-4 py-2 border-b border-white/10 flex justify-between items-center">
                                    <div className="flex items-center gap-2">
                                        <span className="bg-[#3b82f6] text-white text-[10px] font-normal px-2 py-0.5 rounded-sm">{classData.className}</span>
                                        <span className="text-[9px] font-normal text-slate-500 uppercase tracking-wider">{classData.students.length} ÖĞRENCİ</span>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 p-2">
                                    {classData.students.map((item) => (
                                        <div key={item.student.id} className="bg-[#0f172a]/50 hover:bg-[#0f172a] border border-white/5 p-2 flex items-start gap-3 rounded-sm transition-all group">
                                            <div className="text-[10px] font-normal text-slate-500 font-mono bg-black/20 px-1.5 py-0.5 rounded border border-white/5 w-8 text-center shrink-0">
                                                {item.student.number}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="text-[10px] font-normal text-white uppercase truncate mb-1">{item.student.name}</div>
                                                    ...
                                                    {item.records.map((rec, rIdx) => {
                                                        ...
                                                        return (
                                                            <div key={rIdx} className="flex items-center gap-1 bg-red-500/10 border border-red-500/20 px-1.5 py-0.5 rounded-[2px]" title={`${resolvedName} - ${rec.period}. Ders`}>
                                                                <span className="text-[8px] font-normal text-red-500">{rec.period}</span>
                                                                <span className="text-[8px] font-normal text-slate-400 uppercase">{resolvedName}</span>
                                                            </div>
                                                        );
                                                    })}
                                                </div>
                ...
                <div className="flex flex-col items-center justify-center py-24 opacity-20 border border-dashed border-white/10 rounded-sm">
                    ...
                    <span className="text-[10px] font-normal uppercase tracking-[0.2em] text-white">BU TARİHTE DEVAMSIZLIK YOK</span>
                </div>
            )}
        </div>
    );
