        ...
        <div className="space-y-6 animate-in slide-in-from-right-4 duration-500 pb-12">
            ...
            <div className="bg-[#1e293b] border border-red-500/30 p-6 rounded-sm shadow-2xl relative overflow-hidden group">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 z-10 relative">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-[14px] font-normal text-white uppercase tracking-[0.2em]">{studentClass.grade === 8 ? 'LGS' : 'YKS'} KAYIP PUAN MALİYETİ</h3>
                            <p className="text-[10px] font-normal text-slate-400 uppercase mt-1 tracking-widest">HATALARINIZIN SİZE MALİYETİ VE KURTARMA POTANSİYELİ</p>
                        </div>
                    </div>
                    <div className="flex flex-col items-end">
                        <span className="text-[42px] font-normal text-red-500 leading-none">-{totalLostPoints.toFixed(1)}</span>
                        <span className="text-[9px] font-normal text-red-400/60 uppercase tracking-widest mt-1">TOPLAM KAYIP PUAN</span>
                    </div>
                </div>
                ...
                <div className="mt-6 p-4 bg-black/40 border border-white/5 rounded-sm">
                    <div className="flex items-start gap-3">
                        ...
                        <p className="text-[11px] font-normal text-slate-300 leading-relaxed italic">
                            "Merhaba {student.name.split(' ')[0]}, yaptığın son analizlere göre sadece <span className="text-blue-400 font-normal">Matematik</span> ve <span className="text-blue-400 font-normal">Fen</span> branşlarındaki 4 yanlışını düzeltseydin, puanın <span className="text-green-500 font-normal">{student.scoreGoal || '420'}</span> seviyesine çıkabilirdi. Senin için <span className="text-[#fbbf24]">Kazanım Odaklı Telafi Ödevleri</span> hazırladım."
                        </p>
                    </div>
                </div>
            </div>
                ...
                {Object.entries(analysisData).map(([name, data]) => (
                    <div key={name} className="bg-slate-900/60 border border-white/5 p-4 rounded-sm hover:border-[#3b82f6]/40 transition-all group relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-4">
                            <span className="text-[12px] font-normal text-white uppercase tracking-tight">{name}</span>
                            <span className="text-[14px] font-normal text-red-500">-{data.lostPoints.toFixed(1)} P</span>
                        </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="pt-2">
                                <span className="text-[7px] font-normal text-[#3b82f6] uppercase tracking-widest block mb-1">EKSİK KAZANIMLAR</span>
                                <div className="flex flex-wrap gap-1">
                                    {data.objectives.map(obj => (
                                        <span key={obj} className="px-1.5 py-0.5 bg-[#3b82f6]/10 border border-[#3b82f6]/20 text-[#3b82f6] text-[7px] font-normal rounded-sm">{obj}</span>
                                    ))}
                                </div>
                            </div>
                        </div>
            ...
            <div className="bg-[#0f172a] border border-indigo-500/30 p-6 rounded-sm relative overflow-hidden">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="text-[11px] font-normal text-white uppercase tracking-[0.4em]">SENKRON EKOSİSTEM SIRALAMA HAVUZU</h3>
                    <span className="px-2 py-0.5 bg-green-500/10 border border-green-500/20 text-green-500 text-[7px] font-normal uppercase tracking-widest animate-pulse ml-auto">TÜRKİYE GENELİ CANLI</span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-black/40 p-4 border border-white/5 flex flex-col items-center justify-center">
                        <span className="text-[32px] font-normal text-white">412.5</span>
                        <span className="text-[8px] font-normal text-slate-500 uppercase tracking-widest mt-1">GENEL PUAN ORTALAMASI</span>
                    </div>
                    <div className="bg-black/40 p-4 border border-white/5 flex flex-col items-center justify-center">
                        <span className="text-[32px] font-normal text-[#fbbf24]">1,240</span>
                        <span className="text-[8px] font-normal text-slate-500 uppercase tracking-widest mt-1">SİSTEM GENELİ SIRALAMA</span>
                    </div>
                    <div className="bg-black/40 p-4 border border-white/5 flex flex-col items-center justify-center">
                        <span className="text-[32px] font-normal text-green-500">%8.4</span>
                        <span className="text-[8px] font-normal text-slate-500 uppercase tracking-widest mt-1">YÜZDELİK DİLİM (TAHMİNİ)</span>
                    </div>
                </div>
            </div>
        </div>
