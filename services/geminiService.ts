...
// =====================================================
// AI TELAFİ MOTORU - KAZANIM ETİKETLEME
// =====================================================

import { Objective } from '../types';

/**
 * AI ile otomatik kazanım etiketleme.
 * Soru metni ve mevcut kazanım listesi verilerek Gemini en uygun kazanımı eşleştirir.
 * @returns objectiveId ve confidence skoru, hata durumunda null
 */
export const autoTagQuestion = async (
  questionText: string,
  objectives: Objective[]
): Promise<{ objectiveId: string; confidence: number } | null> => {
  if (!questionText.trim() || objectives.length === 0) return null;

  try {
    const ai = getAI();

    const objectiveList = objectives.map(o =>
      `ID: ${o.id} | Kod: ${o.code ?? 'N/A'} | Ders: ${o.subject} | Konu: ${o.topic ?? ''} | Açıklama: ${o.description}`
    ).join('\n');

    const prompt = `GÖREV: Aşağıdaki soru metnini analiz et ve en uygun kazanımı belirle.
...
TALİMATLAR:
1. Soru metnini dikkatlice oku.
2. Kazanım listesindeki her bir kazanımı değerlendir.
3. Sorunun hangi kazanımı ölçtüğünü belirle.
4. Eğer hiçbir kazanım uygun değilse boş döndür.

ÇIKTI FORMATI (JSON):
{
  "objective_id": "kazanımın_id_değeri_veya_boş_string",
  "confidence": 0.95
}`;
    ...
    if (!parsed || !parsed.objective_id) return null;
    ...
    console.error('AI Kazanım Etiketleme Hatası:', error);
    return null;
  }
};

/**
 * Sınav kitapçığı görüntüsünden soruları OCR ile metne çevirir.
 * @returns Soru numarası -> metin eşlemesi
 */
export const extractQuestionsFromImage = async (
  img: string
): Promise<{ questionNumber: number; questionText: string }[]> => {
  try {
    const ai = getAI();
    const base64Data = img.split(',')[1];
    const mimeType = img.split(',')[0].split(':')[1].split(';')[0];

    const prompt = `GÖREV: Bu sınav kitapçığı görüntüsündeki soruları metne çevir.

TALİMATLAR:
1. Her soruyu soru numarasıyla birlikte oku.
2. Soru metni, şıklar ve görsellerin açıklamasını dahil et.
3. Matematiksel ifadeleri düz metin olarak yaz.

ÇIKTI FORMATI (JSON):
{
  "questions": [
    { "questionNumber": 1, "questionText": "soru_metni_ve_şıklar" }
  ]
}`;
